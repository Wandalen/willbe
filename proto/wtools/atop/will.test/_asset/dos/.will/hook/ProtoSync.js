function onModule(context) {
  ...
  if (!config.path.proto || !config.path.module) return null;

  let protoPath = config.path.proto;
  if (!fileProvider.fileExists(protoPath)) return null;
  ...
  if (!fileProvider.fileExists(moduleProtoPath)) return null;
