...
function form3()
{
  ...
  (
    ...
    () => 'Formed reflector should have absolute prefix or none, but source of ' + reflector.absoluteName + ' has ' + _.entity.exportStringCodeShallow( reflector.src.prefixPath )
  );
  _.assert
  (
    reflector.dst.prefixPath === null || path.s.allAreAbsolute( reflector.dst.prefixPath ),
    () => 'Formed reflector should have absolute prefix or none, but destination of ' + reflector.absoluteName + ' has ' + _.entity.exportStringCodeShallow( reflector.src.prefixPath )
  );
