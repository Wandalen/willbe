...
function longOnce( test )
{
  ...
  var got = _.longOnce( dst, src, evaluator );
  ...
  var dst = [ { v : 1 }, { v : 1 }, { v : 2 }, { v : 3 }, { v : 3 }, { v : 4 }, { v : 5 }, { v : 5 } ];
  var src = undefined;
  var got = _.longOnce( dst, evaluator );
  var expected = [ { v : 1 }, { v : 2 }, { v : 3 }, { v : 4 }, { v : 5 } ];
  test.identical( got, expected );
  test.is( got === dst );
  test.is( got !== src );
  ...
  var dst = null;
  var src = [ { v : 2 }, { v : 2 }, { v : 4 }, { v : 4 }, { v : 6 }, { v : 6 } ];
  var got = _.longOnce( dst, src, equalizer );
  var expected = [ { v : 2 }, { v : 4 }, { v : 6 } ];
  test.identical( got, expected );
  test.is( got !== dst );
  test.is( got !== src );
...
function longOnce( test )
{
  ...
  var got  = _.longOnce( dst, function( a )
  {
    return  Math.floor( a );;
  });
