...
function moduleClone( test )
{
  ...
  a.ready.then( ( arg ) =>
  {
    ...
    test.identical( module2.willPath, a.path.join( __dirname, '../will/Exec' ) );
    ...
    test.identical( opener2.openedModule.willPath, a.path.join( __dirname, '../will/Exec' ) );
...
function exportSeveralExports( test )
{
  ...
  .then( ( arg ) =>
  {
    ...
    test.description = 'outfile';
    var outfile = _.fileProvider.configRead( a.abs( 'sub.out/sub.out.will.yml' ) );
    var modulePaths = _.mapKeys( outfile.module );
    var exp = [ 'sub.out', '../sub' ];
    test.identical( _.setFrom( modulePaths ), _.setFrom( exp ) );
    var exported = _.mapKeys( _.select( outfile.module[ outfile.root[ 0 ] ], 'exported/*' ) );
...
function exportSuper( test )
{
  ...
  .then( ( arg ) =>
  {
    ...
    test.description = 'outfile';
    var outfile = _.fileProvider.configRead( a.abs( 'super.out/supermodule.out.will.yml' ) );
    var modulePaths = _.mapKeys( outfile.module );
...
function submodulesDeleteAndDownload( test )
{
  ...
  .then( () =>
  {
    ...
    con.finally( ( err, arg ) =>
    {
      ...
      //   'git+hd://../_repo/ModuleForTesting1?out:./@master',
      //   'npm:///wFiles',
      //   'npm:///wcloner',
      //   'npm:///wstringer',
      //   'npm:///wTesting',
