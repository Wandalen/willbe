...
Завантажимо підмодулі за дефолтним сценарієм (`will .with submodule .build`):
...
Ще одна особливість використання пакетом `will`-файлів полягає в тому, що `willbe` зчитує закінчення `.will.yml` як повне розширення. Скажімо, якщо ви перейменуєте `submodule.will.yml` в `submodule.yml`, то запуск `will .with submodule .build` не знайде потрібний файл:
...
_Порада. Якщо потрібно вказати унікальну назву файла або єдиний аргумент команди в консолі операційної системи використовуйте ґлоби. Наприклад, фраза яка виконає сценарій `nodefault`: `will .with s* .build n*`._
