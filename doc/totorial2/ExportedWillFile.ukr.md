...
```
В сценарії `export` є два критеріони: `default : 1` і `export : 1`. Критеріон `default` - вбудований критеріон, визначає який зі сценаріїв буде виконуватись за замовчуванням, а `export` - позначає, що це сценарій експорту модуля. В полі `steps` внесено `export.single` описаний вище.
