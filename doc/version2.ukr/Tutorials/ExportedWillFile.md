...
Експорт модуля - особливий вид побудови модульної системи результатом виконання якої є згенерований `some_name.out.will`-файл. Експортований `will-файл` містить повну інформацію про експортований модуль та використовується іншими модулями в процесі імпорту підмодулів. Тому, для слідкування за змінами в модулі при побудові експорту модуля в `will-файлі` вказується секція `about` з полями `name` i `version`.  
