...
[Розділені `вілфайли`](WillFileSplit.md) складаються з двох `вілфайлів` і описують один модуль, а у розробника може виникнути потреба помістити `вілфайли` декількох модулів в одній директорії. Для цього використовуються іменовані  `вілфайли`. 

Іменований `вілфайл` - вид `вілфайла`, що має не стандартне ім'я файлу, тобто, починається з буквенних або числових символів. Через коллізію імен в директорії одночасно може знаходитись один неіменований `вілфайл` та необмежена кількість іменованих.   
...
Неіменований `вілфайл` призначений для видалення файлів з директорії `out`, файл `submodules.will.yml` містить посилання на віддалені підмодулі, а файл `export.will.yml` експортує файли з директорії `proto`.
...
Видаліть директорію `out` запустивши збірку побудови за замовчуванням з неіменованого `вілфайла`.  

Робота з іменованими `вілфайлами` відрізняється від неіменованих використанням команди `.with`, її аргументом може бути як повна назва файла, так і назва до розширення `.will.yml`.  
