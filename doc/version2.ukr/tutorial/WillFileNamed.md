...
Для того, щоб `willbe` виколнала операцію з іменованим `will-файлом` використовується команда `.with`. Повний синтаксис цієї команди `will .with [file_name] [command] [argument]`, де `[file_name]` - назва `will-файла`; `[command]` - комнада для іменованого файла; `[argument]` - аргумент команди, якщо він необхідний.   
...
Неіменований `will-файл` призначений для видалення файлів з директорії `out`, файл `submodules.will.yml` містить посилання на віддалені підмодулі, а файл `export.will.yml` експортує файли з директорії `proto`.
