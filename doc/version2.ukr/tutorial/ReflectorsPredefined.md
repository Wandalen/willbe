...
Для використання вбудованих рефлекторів в ресурсі потрібно вказати поле `inherit` з обраним рефлектором. В прикладі, рефлектор `use.predefined.reflector` наслідує вбудований рефлектор `predefined.common`. Запис неповний, його потрібно доповнити інформацією щодо директорій, файли яких будуть фільтруватись, критеріонами за необхідністю, додатковими фільтрами, тощо.  
...
Рефлектор для фільтрації файлів, призначених для релізу проекта. Рефлектор має дві версії: в полі наслідування використовуйте назву `predefined.debug.v1` або `predefined.debug.v2`. Перша версія приймає критеріон `debug` зі значенням `1`, a друга - значення `debug`, відповідно.  
...
Рефлектор, який фільтрує файли підготовки до релізу - відладки, тестові, експериментальні. Рефлектор має дві версії: в полі наслідування використовуйте назву `predefined.release.v1` або `predefined.release.v2`. Перша версія приймає критеріон `debug` зі значенням `0`, a друга зі значенням `release`.  
...
Мультизбірка - особлива збірка побудови, в сценарій якої вносяться інші збірки секції та кроки користувача. Використання мультизбірок полегшує створення комплексних сценаріїв побудови.      
