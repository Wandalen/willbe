...
В `вілфайлі` використовується вбудований крок `predefined.js`, який записано без явного указання наслідування (`inherit : predefined.js`), оскыльки поле `js` присутнє тільки в цьому вбудованому кроці.  В полі `js`  кроку вказуються шляхи до JS-файлів, які будуть виконані. `Willbe` працює лише з експортованими функціями JS-файла тому, функція `hello` в файлі `index.js` експортована.    
