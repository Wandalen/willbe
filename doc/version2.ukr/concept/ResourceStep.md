...
| opts           | додаткові опції, які можуть передаватися через мапу `opts`       |
...
step:                                 # Назва секції
  delete.files:                       # Назва кроку
    inherit: files.delete        # Наслідування
    filePath: some/dir                # Файл, який буде видалено
```

Крок `delete.files` [наслідується](Inheritance.md) від  `files.delete`. Цей крок видаляє файли за шляхом `path::fileToDelete`.

### Вбудований крок `files.reflect`  

Крок для копіювання файлів за допомогою [рефлектора](ResourceReflector.md#Ресурс-рефлектор).

Рефлектори вибирають множину файлів над якою потрібно здійснити якусь операцію. Крок `files.reflect` виконує копіювання файлів із одного місця в інше. За замовчуванням копіювання виконується із збереженням hard-link'ів між парою файлів. Це значить, що файл-призначення та файл-джерело матимуть один і той же зміст та будь-які зміни внесені в один із файлів відображатиметься операційною системою в іншому файлі.
...
step:                                         # Назва секції
  reflect.files:                              # Назва кроку
    inherit: files.reflect               # Наслідування
    reflector: reflector::reflect.some.files  # Рефлектор для вибірки файлів  
    verbosity: 3                              # Рівень вербальності
```

Крок `reflect.files` [наслідується](Inheritance.md) від `files.reflect`. Для вибору файлів крок використовує рефлектор `reflect.some.files`. Рівень вербальності виводу - 3.

### Вбудований крок `js.run`   

Призначений для виконання JavaScript-файлів під час побудови модуля.  

`js.run` має поле `js`, яке вказує шлях до `JS-файла`, що потрібно виконати.

Оскільки поле `js` має лише крок `js.run` - відсутнє в інших вбудованих кроках, то крок `js.run` може бути записаний без явного наслідування.  
...
Крок для виконання `run.js`. Наслідування не явне, так як поле `js` унікальне поле вбудованого кроку `js.run`.

### Вбудований крок `shell.run`  
...
Крок `shell.run` має поля:
- `shell` - команда для виконання операційною системою/
- `currentPath` - шлях до директорії, в якій команду буде виконана. Якщо поле не вказано, то використовується шлях `module.dir`.
...
Поле `shell` присутнє тільки в вбудованому кроці `shell.run` і, тому, крок може бути записаний через неявне наслідування, тобто, з указанням поля `shell` в ресурсі.  
...
Виклик зовнішньої програми `ls -al`. Наслідування від `shell.run` відбувається неявно.

### Вбудований крок `files.transpile`
...
Крок `files.transpile` має поле `reflector` для указання рефлектора за яким буде здійснено перетворення.

```yaml
step:                                           # Назва секції
  transpile.files:                              # Назва кроку
    inherit: files.transpile               # Наслідування
...
Крок `transpile.files` виконує транспіляцію файлів із оптимізацією `debug : release` та конкатенацією `raw : compiled`. Рефлектор `reflect.js.files` здійснює відбір файлів, які потрібно транспілювати та куди помістити результат транспіляції. Рефлектор може мати такий вигляд:  
...
### Вбудований крок `module.export`  

Призначений для виконання особливого виду побудови модуля - експорту. Результатом виконання цього кроку є згенеровані `out-вілфайл` i, опціонально, архів з експортованими файлами.  

Згенерований `out-вілфайл` містить повну інформацію про модуль і експортовані файли, перелік експортованих файлів, версію і т.п.  

Крок `module.export` має поля:
...
Поле `export` присутнє тільки у вбудованому кроці `module.export` і, тому, крок може бути записаний через неявне наслідування, тобто, з указанням поля `export` в ресурсі.
...
При виконанні побудови буде експортовано файли за шляхом `path::proto`, а створення архіву з файлами вимкнено.

### Вбудований крок `file.view`  

Призначений для відкриття локальних або віддалених файлів для перегляду користувачем системи.

Крок `file.view` використовує програми перегляду, що встановлені в операційній системі для перегляду даного типу файлів за замовчуванням. Для запуску перегляду із затримкою використовуйте поле `delay`.

Крок `file.view` має поля:
...
step:                                        # Назва секції
  open.url:                                  # Назва кроку
    inherit: file.view                 # Наслідування
...
- згенеровані файли: експортовані `out-вілфайл` і архів.
