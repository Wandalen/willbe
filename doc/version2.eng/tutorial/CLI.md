...
After [installation](<Instalation.md>) utility `willbe` In the console of your operating system, enter the `will .` command.
...
In the `willbe` utility, all commands start with` will .`. Each command is a phrase composed of one or more words. For example:
...
Copy the code below in the `.will.yml` file:  

In the list of commands of the utility `willbe` is a lot of those that end on` .list`. They make a list of resources of the given type of the current module. Therefore, if you type one of the `. *. дist` commands in the directory where` will-file `is missing, you will receive a warning about the absence of the module.
...
To make sure that the `.about.list` command works with the current module, enter in its  directory without the `will-file`:
...
For descriptive information about the module, use the command `will .about.list`

Please note that some resources have fields with default values. If there is no such a field, loading `will-file` from the disk, utility generates it.
...
Therefore, in the output section of the `will.about.list` command, section `about` has an `enabled` field that does not exist in the `will-file`:
...
To obtain the list of builds of the current module, enter the command `will .builds.list`. Once the result is displayed on the monitor, open the `.will.yml` file with the text editor and compare the contents of the `build` file section and the output of the command.
...
Enumerating the submodules, utility displays a warning message about the error in reading the submodule information and recommends downloading them using the `.submodules.download command`.
...
- Use [the `will.help` command](#command-help) to get the list of commands (phrases).
