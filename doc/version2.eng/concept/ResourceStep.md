...
### Predefined step `predefined.reflect`  
 ...
 `predefined.js` includes one field with a name `js`,which specifies the path to the `JS-file` that has to be executed.

 Since the `js` field has only one step `predefined.js` which is absent in other predefined steps, the `predefined.js` step can be written without explicit inheritance.
